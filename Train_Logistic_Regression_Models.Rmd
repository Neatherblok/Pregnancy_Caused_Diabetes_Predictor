...
mdl.1.fitted.results <- predict(model.1, test.data.normal,type='response')
...
## Train Logistic Regression Model 2
After training Logistic Regression Model 2 we will try out what the difference is with a model that is normalized and still has all features. This model will be trained on all the features from the normalized train data set, with Diagnosis as the dependent variable.
``` {r train_lrm_2}
# Train Logistic Regression model 2
model.2 <- glm(Diagnosis ~.,family=binomial(link='logit'),data=train.data.normal)
```

## Summarize Logistic Regression Model 2
...
## Calculate Metrics for Logistic Regression Model 2
...
mdl.3.fitted.results <- predict(model.3, test.data.not.normal,type='response')
...
Underneath is the relationship between the coefficients and the dependent variable visualization. It shows that the p-value is under 0.05, as the dot in the middle is entirely filled. It also shows the strength of the variables over the x-axis, as **** is further to the right, it means that this variable has a higher influence on the dependent variable than **** does. The whiskers on the left and right represent the confidence interval so the uncertainty of the variance can be shown. Both whiskers are relatively small, which means that there is not a lot of uncertainty of the variance.
``` {r coef_plot}
...
bonferroni_corr <- function(p_value, num_run_test) {
  ...
  if (adjusted.p.value >= adjusted.alpha || is.na(adjusted.p.value)) {
